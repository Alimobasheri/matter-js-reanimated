/**
 * The `MatterReanimated.Runner` module is an optional utility that provides a game loop for running a `MatterReanimated.Engine` inside a browser environment.
 * A runner will continuously update a `MatterReanimated.Engine` whilst synchronising engine updates with the browser frame rate.
...
(function () {
    ...
    /**
     * Runs a `MatterReanimated.Engine` whilst synchronising engine updates with the browser frame rate.
    ...
    Runner.tick = function (runner, engine, time) {
        ...
        if (runner.frameDeltaHistory.length >= 100) {
            if (
                ...
                Common.warnOnce(
                    'MatterReanimated.Runner: runner reached runner.maxUpdates, see docs.'
                );
            } else if (runner.lastUpdatesDeferred) {
                Common.warnOnce(
                    'MatterReanimated.Runner: runner reached runner.maxFrameTime, see docs.'
                );
            }

            if (typeof runner.isFixed !== 'undefined') {
                Common.warnOnce(
                    'MatterReanimated.Runner: runner.isFixed is now redundant, see docs.'
                );
            }

            if (runner.deltaMin || runner.deltaMax) {
                Common.warnOnce(
                    'MatterReanimated.Runner: runner.deltaMin and runner.deltaMax were removed, see docs.'
                );
            }

            if (runner.fps !== 0) {
                Common.warnOnce(
                    'MatterReanimated.Runner: runner.fps was replaced by runner.delta, see docs.'
                );
            }
        }
    };
    ...
    Runner._onNextFrame = function (runner, callback) {
        if (typeof window !== 'undefined' && window.requestAnimationFrame) {
            ...
            throw new Error(
                'MatterReanimated.Runner: missing required global window.requestAnimationFrame.'
            );
        }
    ...
    Runner._cancelNextFrame = function (runner) {
        if (typeof window !== 'undefined' && window.cancelAnimationFrame) {
            ...
            throw new Error(
                'MatterReanimated.Runner: missing required global window.cancelAnimationFrame.'
            );
        }
    };
