 ...
 */
const tickWorklet = (runnerValue, engineId, time) => {
  'worklet';
  ...
  if (!frameDelta || !runnerValue.timeLastTick || frameDelta > Math.max(_maxFrameDelta, runnerValue.maxFrameTime)) {
  ...
  if (runnerValue.frameDeltaSmoothing) {
    ...
    runnerValue.frameDeltaHistory = runnerValue.frameDeltaHistory.slice(-runnerValue.frameDeltaHistorySize);

    // Sort frame delta history
    const deltaHistorySorted = runnerValue.frameDeltaHistory.slice(0).sort((a, b) => a - b);

    // Sample a central window to limit outliers
    const deltaHistoryWindow = deltaHistorySorted.slice(Math.floor(deltaHistorySorted.length * _smoothingLowerBound), Math.floor(deltaHistorySorted.length * _smoothingUpperBound));
  ...
  runnerValue.timeBuffer = _clamp(runnerValue.timeBuffer, 0, runnerValue.frameDelta + engineDelta * _timeBufferMargin);

  // Reset count of over budget updates
  runnerValue.lastUpdatesDeferred = 0;

  // Get max updates per frame
  const maxUpdates = runnerValue.maxUpdates || Math.ceil(runnerValue.maxFrameTime / engineDelta);
  ...
  while (engineDelta > 0 && runnerValue.timeBuffer >= engineDelta * _timeBufferMargin) {
    // Update the engine
    ...
    const elapsedNextEstimate = elapsedTimeTotal + _elapsedNextEstimate * elapsedTimeUpdates / updateCount;

    // Defer updates if over performance budgets for this frame
    if (updateCount >= maxUpdates || elapsedNextEstimate > runnerValue.maxFrameTime) {
      runnerValue.lastUpdatesDeferred = Math.round(Math.max(0, runnerValue.timeBuffer / engineDelta - _timeBufferMargin));
      break;
    }
  }
...
const Runner = ({
  ...
  const runner = (0, _reactNativeReanimated.useSharedValue)({
    delta: 1000 / 60,
    frameDelta: null,
  ...
  const isRunnerInitialized = (0, _reactNativeReanimated.useSharedValue)(false);
