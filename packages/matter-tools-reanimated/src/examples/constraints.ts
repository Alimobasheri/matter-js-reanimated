export const initConstraints = (engine: any) => {
  ...
  var constraint = Constraint.create({
    pointA: { x: 150 * scale, y: 100 * scale },
    bodyB: body,
    pointB: { x: -10 * scale, y: -10 * scale },
  });
  ...
  var constraint2 = Constraint.create({
    pointA: { x: 280 * scale, y: 120 * scale },
    bodyB: body2,
    pointB: { x: -10 * scale, y: -7 * scale },
    stiffness: 0.001,
  });
  ...
  var constraint3 = Constraint.create({
    pointA: { x: 400 * scale, y: 120 * scale },
    bodyB: body3,
    pointB: { x: -10 * scale, y: -10 * scale },
    stiffness: 0.001,
    damping: 0.05,
  ...
  var constraint4 = Constraint.create({
    pointA: { x: 600 * scale, y: 200 * scale },
    bodyB: body4,
    length: 0,
  });

  // add revolute multi-body constraint
  var body5 = Bodies.rectangle(
    500 * scale,
    400 * scale,
    100 * scale,
    20 * scale,
