export const initBouncingBalls = (engine: any) => {
  'worklet';
  ...
  const ground = global.MatterReanimated.Bodies.rectangle(
    width / 2,
    height - 30,
    width,
    60,
  ...
  const leftWall = global.MatterReanimated.Bodies.rectangle(
    0,
    height / 2,
    60,
    height,
  ...
  const rightWall = global.MatterReanimated.Bodies.rectangle(
    width,
    height / 2,
    60,
    height,
  ...
  const balls = Array(100)
    ...
    .map((_, i) => {
      ...
      return global.MatterReanimated.Bodies.circle(
        100 + Math.random() * (width - 200), // Keep balls away from edges
        100 + i * 5,
        radius,
        {
          restitution: 0.8,
  ...
  global.MatterReanimated.World.add(engine.world, [
    ground,
    leftWall,
    rightWall,
    ...balls,
