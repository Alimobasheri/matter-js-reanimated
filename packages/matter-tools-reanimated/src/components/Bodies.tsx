...
export const Bodies: React.FC<RenderProps> = ({ options = {} }) => {
  ...
  useFrameCallback(() => {
    'worklet';
    if (!Array.isArray(global.MatterToolsReanimated.svgContent)) return;

    const newPaths: typeof pathsData.value = {};

    for (const body of global.MatterToolsReanimated.svgContent) {
      ...
      const pathD =
        body.type === 'circle' && body.circleRadius !== undefined
          ? `M ${body.position.x - body.circleRadius},${body.position.y} ` +
